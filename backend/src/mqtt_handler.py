...
async def insert_telemetry(telemetry_data: TelemetryCreate):
    try:
        await db.telemetry.create(data=telemetry_data.dict())
    except Exception as e:
        logging.error(f"Failed to insert telemetry: {e}")

async def insert_battery(battery_data: BatteryCreate):
    try:
        await db.battery.create(data=battery_data.dict())
    except Exception as e:
        logging.error(f"Failed to insert battery data: {e}")

async def insert_camera(camera_data: CameraCreate):
    try:
        await db.camera.create(data=camera_data.dict())
    except Exception as e:
        logging.error(f"Failed to insert camera data: {e}")

async def insert_lidar(lidar_data: LidarCreate):
    try:
        await db.lidar.create(data=lidar_data.dict())
    except Exception as e:
